...
function allgroup($username)
{
    global $conn;

    // Using a prepared statement to prevent SQL injection
    $sql = "SELECT group_details.ToLocation,group_signups.username
    FROM group_details
    LEFT JOIN group_signups ON group_details.Group_ID = group_signups.Group_ID
    LEFT JOIN group_member ON group_member.group_id = group_details.Group_ID
                           AND group_member.member = ?
    WHERE  group_member.request = 'YES' ;";
    // and group_details.End_date < CURRENT_DATE
